package com.ocengiz.survivorbird;
...
public class SurvivorBird extends ApplicationAdapter {
	...
	@Override
	public void create () {                 //oyun acıldıgında olucak method
		...
		birdX = Gdx.graphics.getWidth() / 2 - bird.getHeight() / 2;  		//sonra daha sola cek
	...
	@Override
	public void render () {					//oyun devam ettiginde sureki cagrılan method
		...
		if (gameState == 1){
			...
			for (int i = 0; i < numberOfEnemies; i++){
				...
				if (enemyX[i] < Gdx.graphics.getWidth() / 15){
					...
					enemyOffSet[i] = (random.nextFloat() - 1f) * (Gdx.graphics.getHeight() - 200);
					enemyOffSet2[i] = (random.nextFloat() - 1f) * (Gdx.graphics.getHeight() - 200);
					enemyOffSet3[i] = (random.nextFloat() - 1f) * (Gdx.graphics.getHeight() - 200);
